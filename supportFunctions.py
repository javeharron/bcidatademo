...
def eegFeatureExtraction(df,fs,lowcut,highcut,pcti):
    ...
    for ix in range(0,capper):
        s1=featureExtraction(c1[int(lbnds[ix]):int(ubnds[ix])],fs,lowcut,highcut,pcti)
        s2=featureExtraction(c2[int(lbnds[ix]):int(ubnds[ix])],fs,lowcut,highcut,pcti)
        s3=featureExtraction(c3[int(lbnds[ix]):int(ubnds[ix])],fs,lowcut,highcut,pcti)
        s4=featureExtraction(c4[int(lbnds[ix]):int(ubnds[ix])],fs,lowcut,highcut,pcti)
...
def featureExtraction(data,fs,lowcut,highcut,pcti):
    ...
    featureVector=np.hstack((Psum.flatten(),vrms,peakF,peakFLoc,intensityPcti))
    #print(featureVector)
